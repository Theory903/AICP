...
use commands::{
    handle_agents_slash_command, handle_anthropic_slash_command, handle_github_slash_command,
...
use mammoth_runtime::{
    ...
    route_model, RouterInput, MODEL_AUTO,
    Session, TokenUsage, ToolError, ToolExecutor, UsageTracker,
};
use serde_json::json;
use tools::GlobalToolRegistry;

use aicp::AicpToolExecutor;
...
fn render_cli_error(problem: &str) -> String {
    ...
    lines.push("  Help             mammoth --help".to_string());
    lines.join("\n")
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Serve { port, model, permission_mode } => run_serve(port, &model, permission_mode)?,
        // AICP feature-parity commands
        CliAction::Caps { json } => {
            aicp_cmds::run_caps(json).map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::RunCap { capability, input_json, json } => {
            aicp_cmds::run_capability(&capability, input_json.as_deref(), json)
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::ApprLs { json } => {
            aicp_cmds::run_appr_ls(json).map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::ApprDecide { approval_id, decision, reason } => {
            aicp_cmds::run_appr_decide(&approval_id, &decision, reason.as_deref())
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::Logs { limit, json } => {
            aicp_cmds::run_logs(limit, json).map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::Scan { path, framework } => {
            aicp_cmds::run_scan(path.as_deref(), framework.as_deref())
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::Dev { path, port } => {
            aicp_cmds::run_dev(path.as_deref(), port.as_deref())
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::Policy { subcmd, rest } => {
            aicp_cmds::run_policy(&subcmd, &rest)
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::Map { format, file } => {
            aicp_cmds::run_map(&format, file.as_deref())
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::Test { capability, verbose } => {
            aicp_cmds::run_test(capability.as_deref(), verbose)
                .map_err(Box::<dyn std::error::Error>::from)?;
        }
        CliAction::AicpStatus => {
            aicp_cmds::run_status_check().map_err(Box::<dyn std::error::Error>::from)?;
        }
...
enum CliAction {
    ...
    /// Start the Mammoth Web channel HTTP server.
    Serve {
        port: u16,
        model: String,
        permission_mode: PermissionMode,
    ...
    // --- AICP feature-parity commands ---
    /// List capabilities registered on the AICP runtime.
    Caps { json: bool },
    /// Execute a capability on the AICP runtime.
    RunCap {
        capability: String,
    ...
    Logs { limit: usize, json: bool },
    /// Scan the current workspace for capabilities.
    Scan {
        path: Option<String>,
        framework: Option<String>,
    },
    /// Start the AICP dev server for the current workspace.
    Dev {
        path: Option<String>,
        port: Option<String>,
    },
    /// Manage capability policies (safe/ask/deny/approve/protect/limit).
    Policy { subcmd: String, rest: Vec<String> },
    /// Import capabilities from an external format (openapi/curl/har/postman).
    Map { format: String, file: Option<String> },
    /// Run capability test suite.
    Test {
        capability: Option<String>,
        verbose: bool,
    },
    /// Show AICP runtime health status.
    AicpStatus,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // Mammoth Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Mammoth Code compat: --print makes output non-interactive
    ...
    match rest[0].as_str() {
        ...
        "serve" => {
            let port = rest
                ...
                .unwrap_or(3000);
            Ok(CliAction::Serve { port, model, permission_mode })
        }
        "prompt" => {
        ...
        "caps" | "ls" => {
            ...
            Ok(CliAction::Caps { json })
        }
        "run" => {
            let capability = rest
                .get(1)
                .ok_or_else(|| "usage: mammoth run <capability> [--input '{...}'] [--json]".to_string())?
                .clone();
            ...
            while i < rest.len() {
                match rest[i].as_str() {
                    "--input" | "-i" => {
                        input_json = rest.get(i + 1).cloned();
                        i += 2;
                    }
                    f if f.starts_with("--input=") => {
                        input_json = Some(f[8..].to_string());
                        i += 1;
                    }
                    "--json" | "-j" => { json = true; i += 1; }
                    _ => { i += 1; }
                }
            }
            Ok(CliAction::RunCap { capability, input_json, json })
        }
        "appr" => {
            match rest.get(1).map(String::as_str) {
                Some("ls") | None => {
                    let json = rest.iter().any(|a| a == "--json" || a == "-j");
                    Ok(CliAction::ApprLs { json })
                }
                Some("decide") => {
                    let approval_id = rest
                        .get(2)
                        .ok_or_else(|| "usage: mammoth appr decide <id> <approve|deny> [--reason TEXT]".to_string())?
                        .clone();
                    let decision = rest
                        .get(3)
                        .ok_or_else(|| "usage: mammoth appr decide <id> <approve|deny>".to_string())?
                        .clone();
                    let reason = rest.windows(2).find(|w| w[0] == "--reason").map(|w| w[1].clone());
                    Ok(CliAction::ApprDecide { approval_id, decision, reason })
                }
                Some(sub) => Err(format!("unknown appr subcommand '{sub}' — try ls or decide")),
            }
        }
        "logs" => {
            let limit = rest.windows(2)
                .find(|w| w[0] == "--limit" || w[0] == "-n")
                .and_then(|w| w[1].parse::<usize>().ok())
                .unwrap_or(20);
            let json = rest.iter().any(|a| a == "--json" || a == "-j");
            Ok(CliAction::Logs { limit, json })
        }
        "scan" => {
            let path = rest.get(1).filter(|v| !v.starts_with('-')).cloned();
            let framework = rest.windows(2)
                .find(|w| w[0] == "--framework" || w[0] == "-f")
                .map(|w| w[1].clone());
            Ok(CliAction::Scan { path, framework })
        }
        "dev" => {
            let path = rest.get(1).filter(|v| !v.starts_with('-')).cloned();
            let port = rest.windows(2)
                .find(|w| w[0] == "--port" || w[0] == "-p")
                .map(|w| w[1].clone());
            Ok(CliAction::Dev { path, port })
        }
        "policy" => {
            let subcmd = rest
                .get(1)
                .ok_or_else(|| "usage: mammoth policy <safe|ask|deny|approve|protect|limit> [CAPABILITY]".to_string())?
                .clone();
            let rest_args = rest[2..].to_vec();
            Ok(CliAction::Policy { subcmd, rest: rest_args })
        }
        // Policy shorthand aliases
        "safe" => {
            let cap = rest.get(1).cloned().unwrap_or_default();
            Ok(CliAction::Policy { subcmd: "safe".to_string(), rest: vec![cap] })
        }
        "ask" => {
            let cap = rest.get(1).cloned().unwrap_or_default();
            Ok(CliAction::Policy { subcmd: "ask".to_string(), rest: vec![cap] })
        }
        "deny" => {
            let cap = rest.get(1).cloned().unwrap_or_default();
            Ok(CliAction::Policy { subcmd: "deny".to_string(), rest: vec![cap] })
        }
        "map" => {
            let format = rest
                .get(1)
                .ok_or_else(|| "usage: mammoth map <openapi|curl|har|postman> [FILE]".to_string())?
                .clone();
            let file = rest.get(2).filter(|v| !v.starts_with('-')).cloned();
            Ok(CliAction::Map { format, file })
        }
        "test" => {
            let capability = rest.get(1).filter(|v| !v.starts_with('-')).cloned();
            let verbose = rest.iter().any(|a| a == "--verbose" || a == "-v");
            Ok(CliAction::Test { capability, verbose })
        }
        "status" => Ok(CliAction::AicpStatus),
...
fn parse_direct_slash_cli_action(rest: &[String]) -> Result<CliAction, String> {
    ...
    match SlashCommand::parse(&raw) {
        ...
        Some(command) => Err(format_direct_slash_command_error(
            match &command {
                SlashCommand::Agent { .. } => "/agent".to_string(),
                SlashCommand::Unknown(name) => format!("/{name}"),
                _ => rest[0].clone(),
            }
            .as_str(),
            matches!(command, SlashCommand::Agent { .. } | SlashCommand::Unknown(_)),
        )),
...
fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push("  Try              Start `mammoth` to use interactive slash commands".to_string());
        lines.push(
            "  Tip              Resume-safe commands also work with `mammoth --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
fn default_permission_mode() -> PermissionMode {
    env::var("MAMMOTH_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        authorize_url: String::from("https://platform.mammoth.dev/oauth/authorize"),
        token_url: String::from("https://platform.mammoth.dev/v1/oauth/token"),
        callback_port: None,
        manual_redirect_url: None,
        scopes: vec![
            String::from("user:profile"),
            String::from("user:inference"),
            String::from("user:sessions:mammoth_code"),
        ],
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting Mammoth OAuth login...");
    ...
    let client = MammothApiClient::from_auth(AuthSource::None).with_base_url(api::read_base_url());
    ...
    println!("Mammoth OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("Mammoth OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "Mammoth OAuth login failed. You can close this window."
    } else {
        "Mammoth OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
fn run_resume_command(
    ...
    match command {
        ...
        | SlashCommand::Anthropic { .. }
        | SlashCommand::GitHub { .. }
        | SlashCommand::Session { .. }
        | SlashCommand::Plugins { .. }
        | SlashCommand::Agent { .. } | SlashCommand::Unknown(_) => Err("unsupported resumed slash command".into()),
    }
}
...
struct LiveCli {
    ...
    runtime: ConversationRuntime<DefaultRuntimeClient, AicpToolExecutor<CliToolExecutor>>,
    session: SessionHandle,
    dpc: mammoth_runtime::DpcManager,
    openhands: mammoth_runtime::OpenHandsManager,
    hermes: mammoth_runtime::HermesManager,
}
...
impl LiveCli {
    fn new(
        ...
        // Resolve "auto" model selection before doing anything else.
        let model = route_model(RouterInput {
            requested: model,
            permission_mode,
            prompt_hint: None,
        ...
        let dpc = mammoth_runtime::DpcManager::new(mammoth_runtime::DpcConfig {
            use_p2p: true,
            knowledge_sync: true,
            consensus_threshold: 0.75,
        });
        let openhands = mammoth_runtime::OpenHandsManager::new(mammoth_runtime::OpenHandsConfig {
            enable_microagents: true,
            enable_mcp: true,
        });
        let hermes = mammoth_runtime::HermesManager::new(
            mammoth_runtime::HermesMemoryConfig {
                enabled: true,
                storage_path: ".mammoth/hermes".to_string(),
            },
            vec![],
        );
    ...
    fn startup_banner(&self) -> String {
        ...
        let has_mammoth_md = cwd
            .as_ref()
            .is_some_and(|path| path.join("MAMMOTH.md").is_file());
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
        ...
        if !has_mammoth_md {
            lines.push(
                "  First run        /init scaffolds MAMMOTH.md, .mammoth.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
    ...
    fn anthropic_report(action: Option<&str>, target: Option<&str>) -> Result<String, Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        Ok(handle_anthropic_slash_command(action, target, &cwd)?)
    }

    fn github_report(action: Option<&str>, target: Option<&str>) -> Result<String, Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        Ok(handle_github_slash_command(action, target, &cwd)?)
    }
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Anthropic { action, target } => {
                Self::print_anthropic(action.as_deref(), target.as_deref())?;
                false
            }
            SlashCommand::GitHub { action, target } => {
                Self::print_github(action.as_deref(), target.as_deref())?;
                false
            }
            SlashCommand::Branch { .. } => {
                Self::report_unavailable_repl_command("branch", "git branch commands")
            }
            SlashCommand::Worktree { .. } => {
                Self::report_unavailable_repl_command("worktree", "git worktree commands")
            }
            SlashCommand::CommitPushPr { .. } => Self::report_unavailable_repl_command(
                "commit-push-pr",
                "commit + push + PR automation",
            ),
            SlashCommand::Agent { .. } => {
                eprintln!("{}", render_unknown_repl_command("agent"));
                false
            }
    ...
    fn report_unavailable_repl_command(command: &str, label: &str) -> bool {
        eprintln!("{}", render_mode_unavailable(command, label));
        false
    }
    ...
    fn print_anthropic(action: Option<&str>, target: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        println!("{}", handle_anthropic_slash_command(action, target, &cwd)?);
        Ok(())
    }

    fn print_github(action: Option<&str>, target: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let cwd = env::current_dir()?;
        println!("{}", handle_github_slash_command(action, target, &cwd)?);
        Ok(())
    }
    ...
    fn handle_session_command(
        ...
        let (output, should_reload) = self.session_command_report(action, target)?;
        println!("{}", output);
        Ok(should_reload)
    }

    pub fn session_command_report(
        &mut self,
        action: Option<&str>,
        target: Option<&str>,
        ...
        match action {
            None | Some("list") => {
                Ok((render_session_list(&self.session.id)?, false))
            }
            Some("switch") => {
    ...
    fn handle_plugins_command(
        ...
        let (output, _should_reload) = self.plugins_command_report(action, target)?;
        println!("{}", output);
        Ok(false)
    }

    pub fn plugins_command_report(
        &mut self,
        action: Option<&str>,
        target: Option<&str>,
    ) -> Result<(String, bool), Box<dyn std::error::Error>> {
        ...
        let output = result.message.clone();
        let reload = result.reload_runtime;
        if reload {
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("mammoth-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("mammoth-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("mammoth-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".mammoth").join("sessions");
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No MAMMOTH instruction files discovered in the current directory ancestry.".to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_mammoth_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_mammoth_md()?);
    Ok(())
}

/// Start the Mammoth Web channel HTTP server.
///
/// Binds an axum server on `0.0.0.0:<port>` using the `server` crate.
/// Serves:
///   - `GET  /`                     → Studio supervision console (Mammoth Web channel)
...
///   - `GET  /ext/events`           → Chrome extension SSE bridge
///   - `POST /ext/message`          → Chrome extension inbound message
fn run_serve(port: u16, model: &str, permission_mode: PermissionMode) -> Result<(), Box<dyn std::error::Error>> {
    use std::net::SocketAddr;

    // Resolve "auto" model routing for the web channel.
    let model = route_model(RouterInput {
        requested: model,
        permission_mode,
        prompt_hint: None,
    ...
    // Build the AI inference runner upfront (synchronous, loads config/auth).
    let runner = Arc::new(CliTurnRunner::new(model, permission_mode)?);
    ...
    rt.block_on(async move {
        ...
        eprintln!("Mammoth Web channel listening on http://0.0.0.0:{actual_port}");
        eprintln!("  Studio UI:      http://localhost:{actual_port}/");
        eprintln!("  Health:         http://localhost:{actual_port}/health");
        eprintln!("  Ext bridge SSE: http://localhost:{actual_port}/ext/events");
        let state = AppState::new().with_runner(runner);
        server_serve(listener, state).await
...
// ---------------------------------------------------------------------------
// CliTurnRunner — implements server::TurnRunner using the CLI's inference stack
// ---------------------------------------------------------------------------

/// Implements `server::TurnRunner` for the Mammoth Web channel.
///
/// Each call to `run_turn` builds a fresh `ConversationRuntime` seeded with
/// the provided history, runs one turn, and returns the new assistant messages.
/// This is intentionally stateless per-turn: session state lives in `AppState`.
struct CliTurnRunner {
    model: String,
    permission_mode: PermissionMode,
    system_prompt: Vec<String>,
}
...
impl CliTurnRunner {
    fn new(
        ...
        let system_prompt = build_system_prompt()?;
        Ok(Self { model, permission_mode, system_prompt })
    }
}

impl ServerTurnRunner for CliTurnRunner {
    fn run_turn(
        &self,
        history: Vec<ConversationMessage>,
        user_message: String,
    ) -> Result<Vec<ConversationMessage>, String> {
        // Build a session pre-loaded with existing history (minus the user
        // message we're about to add — run_turn will push it).
        // We pass history without the last user message because run_turn
        // calls session.messages.push(user_input) internally.
        let mut prior = history;
        // The last message is the user message we just stored — strip it so
        // run_turn can re-add it properly.
        if prior.last().is_some_and(|m| m.role == MessageRole::User) {
            prior.pop();
        }
        ...
        let mut runtime = build_runtime(
            ...
            self.system_prompt.clone(),
            true,  // enable_tools
            false, // emit_output — server pushes via SSE, no stdout
            None,  // allowed_tools — all tools enabled
            self.permission_mode,
            None,  // progress_reporter
        )
        .map_err(|e| e.to_string())?;

        // Approve all tool calls without interactive prompt (auto-allow based on
        // permission_mode) — pass None as prompter.
        let summary = runtime
            .run_turn(user_message, None)
            .map_err(|e| e.to_string())?;

        // Collect all new messages: assistant replies and tool results.
...
) -> Result<ConversationRuntime<DefaultRuntimeClient, AicpToolExecutor<CliToolExecutor>>, Box<dyn std::error::Error>>
{
    let (feature_config, tool_registry) = build_runtime_plugin_state()?;
    let inner_executor = CliToolExecutor::new(allowed_tools.clone(), emit_output, tool_registry.clone());
    let aicp_config = feature_config
        .aicp()
        .cloned()
        .unwrap_or_default();
    let tool_executor = AicpToolExecutor::new(inner_executor, &aicp_config);
...
const HELP_QUICK_START: &[&str] = &[
    "  mammoth                                  Start the interactive REPL",
    "  mammoth \"summarize this repo\"            Run one prompt and exit",
    "  mammoth prompt \"explain src/main.rs\"     Explicit one-shot prompt",
    "  mammoth --resume SESSION.json /status    Inspect a saved session",
];

const HELP_INTERACTIVE_ESSENTIALS: &[&str] = &[
...
const HELP_COMMANDS: &[&str] = &[
    "  mammoth dump-manifests                   Read upstream TS sources and print extracted counts",
    "  mammoth bootstrap-plan                   Print the bootstrap phase skeleton",
    "  mammoth agents                           List configured agents",
    "  mammoth skills                           List installed skills",
    "  mammoth system-prompt [--cwd PATH] [--date YYYY-MM-DD]",
    "  mammoth login                            Start the OAuth login flow",
    "  mammoth logout                           Clear saved OAuth credentials",
    "  mammoth init                             Scaffold MAMMOTH.md + local files",
];

const HELP_AICP_COMMANDS: &[&str] = &[
    "  mammoth status                           Check AICP runtime health",
    "  mammoth caps [--json]                    List registered capabilities",
    "  mammoth ls   [--json]                    Alias for caps",
    "  mammoth run  <cap> [--input JSON] [--json]   Execute a capability",
    "  mammoth appr ls  [--json]                List pending approvals",
    "  mammoth appr decide <id> <approve|deny> [--reason TEXT]",
    "  mammoth logs [--limit N] [--json]        Show execution history",
    "  mammoth scan [PATH] [--framework FRAMEWORK]  Scan workspace for capabilities",
    "  mammoth dev  [PATH] [--port PORT]        Start AICP dev server",
    "  mammoth map  <openapi|curl|har|postman> [FILE]  Import capabilities",
    "  mammoth test [CAPABILITY] [--verbose]    Run capability test suite",
    "  mammoth policy <safe|ask|deny|approve|protect|limit> [CAP]",
    "  mammoth safe <cap>   / ask <cap> / deny <cap>  Policy shorthands",
];

const HELP_MODEL_ROUTING: &[&str] = &[
    "  --model auto                          Auto-select model (default)",
    "    Routing: danger-full-access or >60k tokens → opus (flagship)",
    "             coding keywords            → sonnet (balanced)",
    "             quick/explain keywords     → haiku (fast)",
    "  MAMMOTH_MODEL=<model>                 Env-var model override",
    "  AICP_URL=http://host:port             Override AICP runtime URL (default: http://localhost:8000)",
];
...
const HELP_EXAMPLES: &[&str] = &[
    "  mammoth --model opus \"summarize this repo\"",
    "  mammoth --output-format json prompt \"explain src/main.rs\"",
    "  mammoth --allowedTools read,glob \"summarize Cargo.toml\"",
    "  mammoth --resume session.json /status /diff /export notes.txt",
    "  mammoth agents",
    "  mammoth /skills",
    "  mammoth login",
    "  mammoth init",
...
fn write_help_section(out: &mut impl Write, title: &str, lines: &[&str]) -> io::Result<()> {
    writeln!(out, "{title}")?;
    for line in lines {
        writeln!(out, "{line}")?;
    }
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "Mammoth Code CLI v{VERSION}")?;
    writeln!(out, "  Interactive coding assistant for the current workspace.")?;
    writeln!(out)?;
    write_help_section(out, "Quick start", HELP_QUICK_START)?;
    write_help_section(out, "Interactive essentials", HELP_INTERACTIVE_ESSENTIALS)?;
    write_help_section(out, "Commands", HELP_COMMANDS)?;
    write_help_section(
        out,
        "AICP commands  (requires AICP runtime — start with `mammoth dev`)",
        HELP_AICP_COMMANDS,
    )?;
    write_help_section(out, "Model routing", HELP_MODEL_ROUTING)?;
    write_help_section(out, "Flags", HELP_FLAGS)?;
    ...
    write_help_section(out, "Examples", HELP_EXAMPLES)
}
...
fn build_provider_client_supports_openai_compatible_models() {
    ...
    fn shared_help_uses_resume_annotation_copy() {
        ...
        assert!(help.contains("available via mammoth --resume SESSION.json"));
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("mammoth init"));
        assert!(help.contains("mammoth agents"));
        assert!(help.contains("mammoth skills"));
        assert!(help.contains("mammoth /skills"));
    }
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_mammoth_md(std::path::Path::new("."));
        assert!(rendered.contains("# MAMMOTH.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/mammoth-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p mammoth-cli"}"#)
                .contains("cargo test -p mammoth-cli")
        );
        assert_eq!(
