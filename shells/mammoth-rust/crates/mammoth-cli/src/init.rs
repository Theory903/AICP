...
const STARTER_MAMMOTH_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Mammoth Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".mammoth/settings.local.json", ".mammoth/sessions/"];
const ARTIFACT_LABEL_WIDTH: usize = 19;
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let mammoth_dir = cwd.join(".mammoth");
    artifacts.push(InitArtifact {
        name: ".mammoth/",
        status: ensure_dir(&mammoth_dir)?,
    });

    let mammoth_json = cwd.join(".mammoth.json");
    artifacts.push(InitArtifact {
        name: ".mammoth.json",
        status: write_file_if_missing(&mammoth_json, STARTER_MAMMOTH_JSON)?,
    });
    ...
    let mammoth_md = cwd.join("MAMMOTH.md");
    let content = render_init_mammoth_md(cwd);
    artifacts.push(InitArtifact {
        name: "MAMMOTH.md",
        status: write_file_if_missing(&mammoth_md, &content)?,
    });
...
pub(crate) fn render_init_mammoth_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# MAMMOTH.md".to_string(),
        String::new(),
        "This file provides guidance to Mammoth Code (mammothcode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.mammoth.json`; reserve `.mammoth/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `MAMMOTH.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_mammoth_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir(label: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("mammoth-init-{label}-{nanos}-{unique}"))
    }

    #[test]
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        let root = temp_dir("creates-expected-files");
        fs::create_dir_all(root.join("rust")).expect("create rust dir");
        fs::write(root.join("rust").join("Cargo.toml"), "[workspace]\n").expect("write cargo");

        let report = initialize_repo(&root).expect("init should succeed");
        let rendered = report.render();
        assert!(rendered.contains(".mammoth/           created"));
        assert!(rendered.contains(".mammoth.json       created"));
        assert!(rendered.contains(".gitignore          created"));
        assert!(rendered.contains("MAMMOTH.md          created"));
        assert!(root.join(".mammoth").is_dir());
        assert!(root.join(".mammoth.json").is_file());
        assert!(root.join("MAMMOTH.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".mammoth.json")).expect("read mammoth json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".mammoth/settings.local.json"));
        assert!(gitignore.contains(".mammoth/sessions/"));
        let mammoth_md = fs::read_to_string(root.join("MAMMOTH.md")).expect("read mammoth md");
        assert!(mammoth_md.contains("Languages: Rust."));
        assert!(mammoth_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("MAMMOTH.md"), "custom guidance\n").expect("write existing mammoth md");
        fs::write(root.join(".gitignore"), ".mammoth/settings.local.json\n")
            .expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("MAMMOTH.md          skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".mammoth/           skipped (already exists)"));
        assert!(second_rendered.contains(".mammoth.json       skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore          skipped (already exists)"));
        assert!(second_rendered.contains("MAMMOTH.md          skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("MAMMOTH.md")).expect("read existing mammoth md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".mammoth/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".mammoth/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        let root = temp_dir("python-nextjs-markers");
        fs::create_dir_all(&root).expect("create root");
        ...
        let rendered = render_init_mammoth_md(Path::new(&root));
