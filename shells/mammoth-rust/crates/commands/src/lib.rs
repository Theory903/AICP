...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect Mammoth config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded Mammoth instruction memory files",
        argument_hint: None,
        resume_supported: true,
        category: SlashCommandCategory::Workspace,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter MAMMOTH.md for this repo",
        argument_hint: None,
        resume_supported: true,
        category: SlashCommandCategory::Workspace,
    },
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Mammoth Code plugins",
        argument_hint: Some(
    ...
    // Anthropic integration commands
    SlashCommandSpec {
        name: "anthropic",
        aliases: &[],
        summary: "Manage Anthropic skills and plugins",
        argument_hint: Some("[skills|plugins|skill <name>]"),
        resume_supported: false,
        category: SlashCommandCategory::Automation,
    },
    SlashCommandSpec {
        name: "github",
        aliases: &[],
        summary: "Manage GitHub workflows and actions",
        argument_hint: Some("[workflow pr-review|issue-triage|generate <type>]"),
        resume_supported: false,
        category: SlashCommandCategory::Git,
    },
    SlashCommandSpec {
        name: "agent",
        aliases: &[],
        summary: "Integrate GitHub repo as agent capability",
        argument_hint: Some("<repo-url> [--list]"),
        resume_supported: false,
        category: SlashCommandCategory::Automation,
    },
];
...
impl SlashCommand {
    ...
    pub fn parse(input: &str) -> Option<Self> {
        ...
        Some(match command {
            ...
            "agent" => Self::Agent {
                repo_url: parts.next().map(ToOwned::to_owned),
            },
...
pub fn render_slash_command_help() -> String {
    let mut lines = vec![
        ...
        "  [resume] = also available via mammoth --resume SESSION.json".to_string(),
    ];
...
impl DefinitionSource {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::ProjectMammoth => "Project (.mammoth)",
            Self::UserCodexHome => "User ($CODEX_HOME)",
            Self::UserCodex => "User (~/.codex)",
            Self::UserMammoth => "User (~/.mammoth)",
        }
    }
}
...
pub fn handle_commit_push_pr_slash_command(
    ...
    let body_path = write_temp_text_file("mammoth-pr-body", "md", request.pr_body.trim())?;
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectMammoth,
            ancestor.join(".mammoth").join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserMammoth,
            home.join(".mammoth").join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectMammoth,
            ancestor.join(".mammoth").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectMammoth,
            ancestor.join(".mammoth").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserMammoth,
            home.join(".mammoth").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserMammoth,
            home.join(".mammoth").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       mammoth agents".to_string(),
        "  Sources          .codex/agents, .mammoth/agents, $CODEX_HOME/agents".to_string(),
    ];
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       mammoth skills".to_string(),
        "  Sources          .codex/skills, .mammoth/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
pub fn handle_anthropic_slash_command(
    action: Option<&str>,
    target: Option<&str>,
    cwd: &Path,
) -> io::Result<String> {
    match normalize_optional_args(action) {
        None | Some("list") => {
            let mut lines = vec!["Anthropic Modules".to_string(), String::new()];
            lines.push("Available modules:".to_string());
            lines.push("  skills          List Anthropic skills".to_string());
            lines.push("  plugins         List Anthropic plugins".to_string());
            lines.push("  skill <name>    Show skill details".to_string());
            lines.push(String::new());
            lines.push("Use /anthropic <module> to interact with a module.".to_string());
            Ok(lines.join("\n"))
        }
        Some("skills") => {
            let study_path = cwd.join("ref").join("study").join("skills");
            if !study_path.exists() {
                return Ok("Anthropic Skills\n  Status           not found\n  Path             ref/study/skills".to_string());
            }
            let mut lines = vec!["Anthropic Skills".to_string()];
            let mut count = 0;
            if let Ok(entries) = fs::read_dir(&study_path) {
            ...
            if count == 0 {
                ...
                lines.insert(2, format!("  {count} available skills"));
            }
            Ok(lines.join("\n"))
        }
        Some("plugins") => {
            let study_path = cwd.join("ref").join("study").join("knowledge-work-plugins");
            if !study_path.exists() {
                return Ok("Anthropic Plugins\n  Status           not found\n  Path             ref/study/knowledge-work-plugins".to_string());
            }
            let mut lines = vec!["Anthropic Plugins".to_string()];
            let mut count = 0;
            if let Ok(entries) = fs::read_dir(&study_path) {
                for entry in entries.flatten() {
                    if entry.path().is_dir() {
            ...
            if count == 0 {
                ...
                lines.insert(2, format!("  {count} available plugins"));
            }
            Ok(lines.join("\n"))
        }
        Some("skill") => {
            let Some(name) = target else {
                return Ok(
                    "Anthropic Skill\n  Usage            /anthropic skill <name>".to_string(),
                );
            };
            let skill_path = cwd
                .join("ref")
        ...
        Some(other) => Ok(format!(
            "Anthropic\n  Unknown module  {other}\n  Use /anthropic list to see available modules."
        )),
    }
}

pub fn handle_github_slash_command(
    action: Option<&str>,
    target: Option<&str>,
    cwd: &Path,
) -> io::Result<String> {
    match normalize_optional_args(action) {
        None | Some("list") => {
            let mut lines = vec!["GitHub Integration".to_string(), String::new()];
            lines.push("Available commands:".to_string());
            lines.push("  workflow pr-review     Create PR review workflow".to_string());
            lines.push("  workflow issue-triage Create issue triage workflow".to_string());
            lines.push(
                "  generate <type>        Generate workflow (pr-review|issue-triage)".to_string(),
            );
            lines.push(String::new());
            lines.push("Use /github workflow <type> to create a workflow.".to_string());
            Ok(lines.join("\n"))
        }
        Some("workflow") => match target {
            Some("pr-review") => {
                let workflow_path = cwd
                    .join(".github")
                    .join("workflows")
                    .join("claude-pr-review.yml");
                let content = r#"name: Claude PR Review
  ...
  review:
    ...
    steps:
      ...
      - name: Run Claude Code
        ...
        with:
          prompt: Review this pull request for bugs, security issues, code quality.
          mode: review
        env:
...
"#;
            ...
            Some("issue-triage") => {
                let workflow_path = cwd
                    .join(".github")
                    .join("workflows")
                    .join("claude-issue-triage.yml");
                let content = r#"name: Claude Issue Triage
  ...
  triage:
    ...
    steps:
      - name: Run Claude Code
        uses: anthropics/claude-code-action@v1
        with:
          prompt: Analyze this issue. Add labels, prioritize, and provide initial assessment.
          mode: question
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
"#;
            ...
            _ => Ok(
                "GitHub Workflow\n  Usage            /github workflow pr-review|issue-triage"
                    .to_string(),
            ),
        },
        Some("generate") => {
            let Some(workflow_type) = target else {
                return Ok(
                    "GitHub Generate\n  Usage            /github generate pr-review|issue-triage"
                        .to_string(),
                );
            };
            handle_github_slash_command(Some("workflow"), Some(workflow_type), cwd)
        }
        Some(other) => Ok(format!(
            "GitHub\n  Unknown command {other}\n  Use /github list to see available commands."
        )),
    }
}

pub fn handle_agent_slash_command(repo_url: Option<&str>, cwd: &Path) -> io::Result<String> {
    match repo_url {
        None => Ok(
            "Agent Integration\n  Usage              /agent <github-repo-url> [--list]\n  Examples:\n    /agent NousResearch/hermes-agent\n    /agent https://github.com/anthropic/claude-code-action\n    /agent --list\n  \n  Auto-detects LLM provider from environment (OPENAI_API_KEY, ANTHROPIC_API_KEY, etc.)\n  Creates agent.md and registers with AICP automatically.".to_string(),
        ),
        Some("--list") => {
            let registry_path = cwd.join(".aicp-agents.json");
            if registry_path.exists() {
                let content = fs::read_to_string(&registry_path)?;
                return Ok(format!("Registered Agents:\n{}", content));
            }
            Ok("No agents registered yet.\nUse /agent <repo-url> to integrate.".to_string())
        }
        Some(url) if url.starts_with("--") => Ok("Use /agent --list to see registered agents.".to_string()),
        Some(url) => {
            let clean_url = url.replace(".git", "");
            let agent_path = cwd.join("agents").join(format!(
                "aicp_{}.md",
                clean_url.replace("/", "_")
            ));
            if agent_path.exists() {
                ...
                return Ok(format!(
                    ...
                    clean_url, agent_path.display(), content
                ));
            }
            Ok(format!(
                "Agent Integration\n  Repository {}\n  Status    Ready to integrate\n\nTo integrate, run:\n  python -c \"from aicp.agents import auto_integrate; auto_integrate('{}')\"",
                clean_url, clean_url
            ))
        }
    }
...
mod tests {
    ...
    fn init_git_repo(label: &str) -> PathBuf {
        ...
        run_command(&root, "git", &["config", "user.name", "Mammoth Tests"]);
        run_command(
            &root,
            "git",
            &["config", "user.email", "mammoth@example.com"],
        );
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        fs::create_dir_all(root.join(".mammoth-plugin")).expect("manifest dir");
        fs::write(
            root.join(".mammoth-plugin").join("plugin.json"),
            format!(
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".mammoth").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectMammoth,
        ...
        assert!(report.contains("Project (.mammoth):"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       mammoth agents"));
