...
if TYPE_CHECKING:
    from aicp.managers.agent_manager import DpcAgentManager
    from aicp.service import CoreService
...
def parse_thinking_tags(content: str) -> tuple[str, str | None]:
    r"""
...
class OllamaProvider(AIProvider):
    ...
    @staticmethod
    def _parse_num_ctx_from_modelfile(modelfile: str) -> int | None:
    ...
    async def generate_response(self, prompt: str, **kwargs) -> str:
        try:
            ...
            if final_text:
                ...
                logger.warning("No text block found in response, only thinking blocks")
                return ""
            else:
                return ""
    ...
    async def generate_response_stream(
        ...
        try:
            ...
            if "Event loop is closed" in str(e):
                logger.debug("Z.AI streaming cleanup skipped (event loop closed)")
    ...
    def _unload_model(self) -> None:
        ...
        try:
            ...
            # Handle PyTorch backend (CUDA/MPS/CPU)
                ...
                if device and 'cuda' in device or device and 'mps' in device:
...
class DpcAgentProvider(AIProvider):
    ...
    async def _ensure_manager(self, agent_id: str | None = None) -> "DpcAgentManager":
        """
...
class LLMManager:
    ...
    def __init__(self, config_path: Path = Path.home() / ".dpc" / "providers.json"):
        ...
        self._on_providers_reload_callback: Callable[[], None] | None = None

        # Token counting manager (Phase 4 refactor - v0.12.1)
        from aicp.managers.token_count_manager import TokenCountManager
    ...
    async def query(self, prompt: str, provider_alias: str | None = None, return_metadata: bool = False,
                    images: list[dict[str, Any]] | None = None, **kwargs):
        """
