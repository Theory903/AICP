...
class FederationNode(BaseModel):
    ...
    @classmethod
    def _validate_last_seen(cls, value: str | None) -> str | None:
        if value is None:
            return None
...
class FederationProvider(ABC):
    @abstractmethod
    async def discover_node(self, url: str) -> FederationNode | None:
...
class DIDAuthenticator:
    ...
    def authenticate(self, did: str, challenge: str, signature: str) -> bool:
        ...
        if not challenge or not signature:
            return False
        expected_signature = hashlib.sha256(f"{did}:{challenge}".encode()).hexdigest()
