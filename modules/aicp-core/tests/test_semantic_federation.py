...
async def test_sync_registry_fetches_remote_registry() -> None:
    ...
    assert registry.id == "reg_123"
    assert registry.name == "mesh"
    assert registry.node_id == "remote-node"
    assert registry.get("notes.create") == {"version": 1}
    assert registry.vector_clock == {"remote-node": 3}
    assert registry.last_updated == "2026-04-05T10:00:00"
...
async def test_sync_registry_without_endpoint_returns_empty_registry() -> None:
    ...
    assert registry.node_id == "local"
    assert registry.entries == {}
    assert registry.vector_clock == {}
    datetime.fromisoformat(registry.last_updated)
