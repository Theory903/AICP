...
class CompactionService:
    ...
    async def compact_session(
        ...
        *,
        ...
        compacted_messages, preserved_entries, compacted_entries = (
        ...
        await self._append_audit(
            session=session, result=result, entries=compacted_messages
        )
        return result
    ...
    async def get_budget_status(self, session_id: str) -> BudgetStatus:
        normalized_session_id = self._require_text(session_id, "session_id")
        session = await self._load_session(normalized_session_id)
        return self._build_budget_status(
            session=session, mode=self._session_mode(session)
        )

    async def set_budget_limit(
        self, session_id: str, token_limit: int
    ) -> BudgetLimitUpdate:
        normalized_session_id = self._require_text(session_id, "session_id")
        if token_limit < 1:
    ...
    def _relevance_score(self, entry: dict[str, Any]) -> float:
        ...
        if any(
            word in lowered
            for word in {"decide", "because", "plan", "result", "output"}
        ):
            score += 0.45
        if any(
            word in lowered for word in {"hello", "thanks", "ok", "sure", "confirmed"}
        ):
            score -= 0.1
    ...
    def _entry_key(self, entry: dict[str, Any]) -> str:
        entry_type = str(entry.get("type") or "message")
        content = (
            str(entry.get("content") or entry.get("decision") or "").strip().lower()
        )
