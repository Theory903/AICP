...
class WebRTCPeerConnection:
    ...
    def __init__(self, node_id: str, is_initiator: bool = False):
        ...
        if turn_username and turn_credential:
            ...
            if turn_urls:
                ...
                logger.info(
                    "Using configured TURN servers (%d URLs) with username: %s...",
                    len(turn_urls),
                    turn_username[:8],
                )
            else:
                logger.warning(
                    "TURN credentials provided but no TURN server URLs in config.ini [turn] servers"
                )
        else:
            logger.warning(
                "No TURN credentials configured - set DPC_TURN_USERNAME and DPC_TURN_CREDENTIAL environment variables or add [turn] username/credential to ~/.dpc/config.ini - WebRTC connections may fail without TURN relay"
            )
            ...
            if fallback_urls and fallback_username and fallback_credential:
                ...
                logger.info(
                    "Using fallback public TURN servers (%d URLs) - may be unreliable",
                    len(fallback_urls),
                )
            else:
        ...
        self._keepalive_interval = (
            5.0  # Send keepalive every 5 seconds (aggressive to prevent SCTP timeout)
        )
    ...
    def _setup_handlers(self):
        ...
        def on_datachannel(channel: RTCDataChannel):
            ...
            logger.info(
                "Data channel '%s' received from peer %s", channel.label, self.node_id
            )
        ...
        async def on_icegatheringstatechange():
            ...
            if state == "complete":
                ...
                if self.pc.localDescription:
                    ...
                    logger.info(
                        "[%s] ICE gathering complete - host:%d srflx:%d relay:%d",
                        self.node_id,
                        host_count,
                        srflx_count,
                        relay_count,
                    ...
                    logger.info(
                        "[%s] ICE gathering complete - SDP includes all candidates",
                        self.node_id,
                    )
        ...
        async def on_iceconnectionstatechange():
            ...
            if state == "completed":
                ...
                try:
                    ...
                    if (
                        hasattr(self.pc, "_RTCPeerConnection__sctp")
                        and self.pc._RTCPeerConnection__sctp
                    ):
                        ...
                        if hasattr(transport, "_connection") and transport._connection:
                            ...
                            if selected:
                                local_type = (
                                    selected[0].type if selected[0] else "unknown"
                                )
                                remote_type = (
                                    selected[1].type if selected[1] else "unknown"
                                )
                                local_addr = (
                                    f"{selected[0].host}:{selected[0].port}"
                                    if selected[0]
                                    else "unknown"
                                )
                                remote_addr = (
                                    f"{selected[1].host}:{selected[1].port}"
                                    if selected[1]
                                    else "unknown"
                                )
                                logger.info(
                                    "[%s] Selected ICE candidate pair - Local: %s @ %s, Remote: %s @ %s",
                                    self.node_id,
                                    local_type,
                                    local_addr,
                                    remote_type,
                ...
                logger.error(
                    "[%s] ICE connection FAILED - NAT traversal unsuccessful - Possible causes: Symmetric NAT requiring TURN relay, Firewall blocking UDP traffic, TURN server unavailable or misconfigured",
                    self.node_id,
                )
            elif state == "disconnected":
                logger.warning(
                    "[%s] ICE connection DISCONNECTED - network change or timeout - Connection may recover automatically",
                    self.node_id,
                )
            elif state == "closed":
        ...
        async def on_connectionstatechange():
            ...
            if state == "connected":
                ...
                if not self._closing:
                    ...
                    logger.info(
                        "[%s] WebRTC connection closed (intentional)", self.node_id
                    )
    ...
    def _setup_channel_handlers(self):
        ...
        def on_close():
            ...
            if not self._closing:
                logger.warning(
                    "Data channel closed with %s (unexpected) - ICE state: %s, Connection state: %s, DC state: %s",
                    self.node_id,
                    ice_state,
                    conn_state,
                    dc_state,
    ...
    async def create_offer(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "[%s] ICE gathering timeout after 10s, sending SDP anyway", self.node_id
            )
        ...
        if self.pc.localDescription:
            ...
            relay_count = sdp.count("typ relay")
            logger.info(
                "[%s] Offer SDP contains - host:%d srflx:%d relay:%d",
                self.node_id,
                host_count,
                srflx_count,
                relay_count,
        ...
        logger.info(
            "[%s] Offer created with %d bytes SDP",
            self.node_id,
            len(self.pc.localDescription.sdp),
        )
    ...
    async def handle_answer(self, answer: Dict[str, Any]):
        ...
        logger.info(
            "[%s] Remote description set, ICE checking will begin", self.node_id
        )
    ...
    async def handle_offer(self, offer: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.pc.localDescription:
            ...
            relay_count = sdp.count("typ relay")
            logger.info(
                "[%s] Answer SDP contains - host:%d srflx:%d relay:%d",
                self.node_id,
                host_count,
                srflx_count,
                relay_count,
        ...
        logger.info(
            "[%s] Answer created with %d bytes SDP",
            self.node_id,
            len(self.pc.localDescription.sdp),
        )
    ...
    def _diagnose_connection_failure(self) -> str:
        ...
        if srflx_count == 0:
            ...
            diagnosis_parts.append(
                f"✓ STUN working - {srflx_count} server reflexive candidate(s) found"
            )
        ...
        if relay_count == 0:
            ...
            diagnosis_parts.append(
                f"✓ TURN working - {relay_count} relay candidate(s) found"
            )
        ...
        if relay_count == 0:
            recommendations.append(
                "1. Configure TURN credentials (highest priority for symmetric NAT)"
            )
        if srflx_count == 0:
            recommendations.append(
                "2. Check STUN server connectivity (verify internet connection)"
            )
        if ice_state == "checking" and relay_count > 0:
            recommendations.append(
                "3. Check firewall/router UDP port settings (TURN ports may be blocked)"
            )
    ...
    def _start_keepalive(self):
        ...
        if self._keepalive_task is None or self._keepalive_task.done():
            ...
            logger.debug(
                "[%s] Keepalive task started (interval: %ss)",
                self.node_id,
                self._keepalive_interval,
            )
    ...
    async def _keepalive_loop(self):
        ...
        try:
            while True:
                ...
                if not self.data_channel or self.data_channel.readyState != "open":
                    logger.debug(
                        "[%s] Data channel not open, stopping keepalive", self.node_id
                    )
                    break
                ...
                try:
                    ...
                    if ping_count % 5 == 0:
                        logger.debug(
                            "[%s] Keepalive ping #%d sent", self.node_id, ping_count
                        )

                except Exception as e:
                    logger.error(
                        "[%s] Error sending keepalive ping #%d: %s",
                        self.node_id,
                        ping_count,
                        e,
                    )
            ...
            logger.debug(
                "[%s] Keepalive cancelled (sent %d pings)", self.node_id, ping_count
            )
        except Exception as e:
            logger.error(
                "[%s] Keepalive loop error: %s", self.node_id, e, exc_info=True
            )
    ...
    def get_external_ip(self) -> str | None:
        ...
        try:
            ...
            srflx_pattern = (
                r"a=candidate:\S+ \S+ \S+ \S+ (\d+\.\d+\.\d+\.\d+) \S+ typ srflx"
            )
            ...
            logger.warning(
                "[%s] Error extracting external IP from SDP: %s", self.node_id, e
            )
