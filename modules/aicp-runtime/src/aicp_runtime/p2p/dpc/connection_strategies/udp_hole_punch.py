...
class UDPHolePunchStrategy(ConnectionStrategy):
    ...
    async def connect(
        ...
        if (
            not hasattr(orchestrator, "hole_punch_manager")
            or not orchestrator.hole_punch_manager
        ):
        ...
        logger.info(
            ...
            node_id[:20],
            endpoints.punch.nat_type if endpoints.punch else "unknown",
        )

        try:
            # Step 1: Discover our external endpoint
            local_endpoint = (
                await orchestrator.hole_punch_manager.discover_external_endpoint()
            )
            ...
            if local_endpoint.nat_type == "symmetric":
                ...
                raise StrategyNotApplicableError(
                    "Symmetric NAT not supported by hole punching"
                )
            ...
            logger.info(
                ...
                local_endpoint.ip,
                local_endpoint.port,
                local_endpoint.nat_type,
                local_endpoint.confidence * 100,
            )
            ...
            logger.info(
                "UDP hole punch successful to %s, upgrading to DTLS...", node_id[:20]
            )
            ...
            try:
                ...
                peer_connection = UDPPeerConnection(
                    node_id=node_id, dtls_conn=dtls_conn
                )

                logger.info(
                    "UDP hole punch + DTLS successful to %s (encrypted)", node_id[:20]
                )
            ...
            logger.warning(
                "UDP hole punch timeout to %s (after %.1fs)", node_id[:20], self.timeout
            )
            raise ConnectionError(f"Hole punch timeout to {node_id[:20]}")
        except Exception as e:
            logger.warning("UDP hole punch failed to %s: %s", node_id[:20], e)
