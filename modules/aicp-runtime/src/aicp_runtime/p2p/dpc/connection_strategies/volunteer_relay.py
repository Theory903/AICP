    ...
    async def connect(
        ...
        try:
            ...
            relay_node = await asyncio.wait_for(
                orchestrator.relay_manager.find_relay(), timeout=10.0
            )
            ...
            logger.info(
                ...
                relay_node.node_id[:20],
                relay_node.region,
                relay_node.quality_score(),
            )
            ...
            relayed_conn = await orchestrator.relay_manager.connect_via_relay(
                peer_id=node_id, relay_node=relay_node
            )
            ...
            logger.warning("Relay connection failed to %s: %s", node_id[:20], e)
