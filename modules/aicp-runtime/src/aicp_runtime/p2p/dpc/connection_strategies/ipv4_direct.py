    ...
    async def connect(
        ...
        for addr_type, address in addresses_to_try:
            try:
                ...
                logger.info(
                    "IPv4 %s connection established to %s", addr_type, node_id[:20]
                )
                return connection

            except asyncio.TimeoutError:
                logger.debug("IPv4 %s connection timeout to %s:%d", addr_type, ip, port)
                last_error = ConnectionError(f"IPv4 {addr_type} timeout")
            except Exception as e:
                logger.debug(
                    "IPv4 %s connection failed to %s:%d: %s", addr_type, ip, port, e
                )
                last_error = e
