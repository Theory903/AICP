    ...
    async def close(self):
        ...
        if not self.writer.is_closing():
            ...
            try:
                ...
                logger.debug(
                    "Error during SSL shutdown for peer %s: %s", self.node_id, e
                )
...
class P2PManager:
    ...
    def __init__(self, firewall: ContextFirewall, settings=None):
        ...
        self._hub_client_refs: Dict[
            str, HubClient
        ] = {}  # Store hub_client for reconnection

        # Background task tracking for graceful shutdown
        self._peer_listener_tasks: Dict[
            str, asyncio.Task
        ] = {}  # Track _listen_to_peer tasks
        ...
        logger.info(
            "Peer cache initialized with %d cached peers",
            len(self.peer_cache.get_all_peers()),
        )
        ...
        self.on_peer_disconnected: Callable | None = (
            None  # Callback for peer disconnection
        )
    ...
    async def start_server(self, host: str = "0.0.0.0", port: int = 8888):
        ...
        if host == "dual":
            ...
            if is_windows:
                ...
                logger.info(
                    "Windows detected - creating separate IPv4 and IPv6 listeners"
                )
                ...
                try:
                    ...
                    self._server_task = asyncio.gather(
                        server_v4.serve_forever(), server_v6.serve_forever()
                    )
                    logger.info(
                        "P2PManager Direct TLS server listening on 0.0.0.0:%d and [::]:%d (dual-stack) for node %s",
                        port,
                        port,
                        self.node_id,
                    )
                ...
                logger.info(
                    "Linux/macOS detected - creating separate IPv4 and IPv6 listeners"
                )
            ...
            logger.info(
                "P2PManager Direct TLS server listening on %s:%d for node %s",
                formatted_host,
                port,
                self.node_id,
            )
        ...
        if self.settings.get_dht_enabled():
            try:
                ...
                logger.info(
                    "DHT started on UDP port %d for node %s", dht_port, self.node_id
                )
    ...
    async def connect_via_node_id(
        self, target_node_id: str, timeout: float = None
    ) -> bool:
        """
        ...
        if (
            cached_peer
            and cached_peer.last_direct_ip
            and cached_peer.is_recently_seen(hours=168)
        ):  # 7 days
            logger.info(
            ...
            try:
                ...
                logger.info(
                    "Connected to %s via peer cache (no DHT lookup needed)",
                    target_node_id[:20],
                )
        ...
        logger.error(
            "Failed to connect to %s - All connection methods failed", target_node_id
        )
        return False

    async def _handle_direct_connection(
        ...
        try:
            ...
            if not self._verify_hello_identity(
                peer_node_id, cert_pem, nonce, nonce_signature_b64
            ):
                raise ConnectionError(
            ...
            if peer_name:
                ...
                if hasattr(self, "_core_service_ref") and self._core_service_ref:
                    self._core_service_ref.set_peer_metadata(
                        peer_node_id, name=peer_name
                    )
    ...
    async def connect_directly(
        self, host: str, port: int, target_node_id: str, timeout: float = None
    ):
        """
        ...
        if timeout is None:
            timeout = (
                self.settings.get_p2p_connection_timeout() if self.settings else 60.0
            )

        logger.info(
            "Initiating direct connection to %s at %s:%d", target_node_id, host, port
        )
        ...
        preflight_timeout = (
            60.0  # Increased for high-latency networks (mobile carriers, CGNAT)
        )
        port_accessible, port_message = await self.test_port_connectivity(
            host, port, preflight_timeout
        )
        ...
        try:
            ...
            try:
                await self.send_message_to_peer(
                    target_node_id, create_get_providers_message()
                )
                logger.debug("Requested AI providers from %s", target_node_id)
            except Exception as e:
                logger.warning(
                    "Failed to request providers from %s: %s", target_node_id, e
                )
            ...
            logger.error(
                "Failed to establish direct connection with %s: %s",
                target_node_id,
                e,
                exc_info=True,
            )
    ...
    def _validate_peer_certificate(self, cert: Any, expected_node_id: str) -> bool:
        """
    ...
    def _verify_hello_identity(
        self, peer_node_id: str, cert_pem: str, nonce: bytes, nonce_signature_b64: str
    ) -> bool:
        """
        ...
        try:
            ...
            logger.debug(
                "HELLO identity verified for %s (all 3 checks passed)", peer_node_id
            )
            return True

        except Exception as e:
            logger.warning(
                "HELLO identity verification failed for %s: %s", peer_node_id, e
            )
            return False
    ...
    async def _listen_to_peer(self, peer: PeerConnection):
        ...
        try:
            ...
            logger.debug(
                "Listener task for peer %s cancelled during shutdown", peer.node_id
            )
    ...
    def _create_webrtc_close_handler(self, node_id: str):
        ...
        async def handle_close(_node_id):
            ...
            if node_id not in self._intentional_disconnects:
                logger.info(
                    "Auto-Reconnect: Connection to %s lost unexpectedly", node_id
                )
                ...
                if (
                    stored_hub_client
                    and stored_hub_client.websocket
                    and stored_hub_client.websocket.state == websockets.State.OPEN
                ):
                    try:
                        ...
                        logger.error(
                            "Auto-Reconnect: Failed to reconnect to %s: %s", node_id, e
                        )
                else:
                    logger.warning(
                        "Auto-Reconnect: Cannot reconnect to %s - Hub client not connected",
                        node_id,
                    )
                    self._hub_client_refs.pop(node_id, None)
            else:
                logger.info(
                    "Auto-Reconnect: Disconnection was intentional, not reconnecting"
                )
    ...
    async def connect_via_hub(self, target_node_id: str, hub_client: HubClient):
        ...
        try:
            ...
            webrtc_peer = WebRTCPeerConnection(
                node_id=target_node_id, is_initiator=True
            )
            self._pending_webrtc[target_node_id] = webrtc_peer

            # Set up ICE candidate handler
            async def handle_ice(candidate_dict):
                await hub_client.send_signal(
                    target_node_id,
                    {"type": "ice-candidate", "candidate": candidate_dict},
                )
            ...
            async def handle_message(message):
                if self.on_message_received:
                    asyncio.create_task(
                        self.on_message_received(target_node_id, message)
                    )
            ...
            await hub_client.send_signal(
                target_node_id, {"type": "offer", "offer": offer}
            )
            ...
            logger.error(
                "Failed to initiate WebRTC connection to %s: %s",
                target_node_id,
                e,
                exc_info=True,
            )
    ...
    async def handle_incoming_signal(
        self, signal: Dict[str, Any], hub_client: HubClient
    ):
        """
        ...
        if signal_type == "signal":
            ...
            if not from_node or not webrtc_signal_type:
                logger.warning(
                    "Received invalid Hub signal (missing sender_node_id or payload.type): %s",
                    signal,
                )
                return

            logger.debug(
                "Received WebRTC signal '%s' from %s", webrtc_signal_type, from_node
            )

            try:
                if webrtc_signal_type == "offer":
                    await self._handle_webrtc_offer(
                        from_node, wrapped_payload, hub_client
                    )
                ...
                logger.error(
                    "Error handling signal from %s: %s", from_node, e, exc_info=True
                )

        else:
            # Unknown signal format
            logger.warning(
                "Received unknown signal format (type=%s): %s", signal_type, signal
            )

    async def _handle_webrtc_offer(
        self, from_node: str, payload: Dict[str, Any], hub_client: HubClient
    ):
        ...
        async def handle_ice(candidate_dict):
            await hub_client.send_signal(
                from_node, {"type": "ice-candidate", "candidate": candidate_dict}
            )
        ...
        # Send answer back
        await hub_client.send_signal(from_node, {"type": "answer", "answer": answer})
    ...
    async def _handle_ice_candidate(self, from_node: str, payload: Dict[str, Any]):
        ...
        if webrtc_peer and isinstance(webrtc_peer, WebRTCPeerConnection):
            ...
            logger.debug(
                "Buffered ICE candidate from %s (total: %d)",
                from_node,
                len(self._ice_candidates_buffer[from_node]),
            )
    ...
    async def _finalize_webrtc_connection(self, node_id: str):
        ...
        try:
            ...
            logger.error(
                "Failed to finalize WebRTC connection with %s: %s",
                node_id,
                e,
                exc_info=True,
            )
    ...
    async def _handle_connection_timeout(self, node_id: str):
        ...
        try:
            ...
            if node_id in self._pending_webrtc and node_id not in self.peers:
                logger.warning(
                    "WebRTC connection to %s timed out after %.1f seconds",
                    node_id,
                    timeout,
                )
        ...
        for peer_id in webrtc_peers_to_restart:
            try:
                ...
                if peer_id in self.peers:
                    ...
                    await peer.close()
                    logger.debug(
                        "Closed active WebRTC connection with %s", peer_id[:20]
                    )
                ...
                if peer_id in self._pending_webrtc:
                    ...
                    logger.debug(
                        "Closed pending WebRTC connection with %s", peer_id[:20]
                    )
                ...
                logger.error(
                    "Error closing WebRTC connection with %s: %s", peer_id[:20], e
                )
    ...
    async def shutdown_peer_connection(self, peer_id: str):
        ...
        if self.on_peer_disconnected:
            try:
                ...
                logger.error(
                    f"Error in on_peer_disconnected callback for {peer_id}: {e}",
                    exc_info=True,
                )
    ...
    async def shutdown_all(self):
        ...
        if self._peer_listener_tasks:
            await asyncio.gather(
                *self._peer_listener_tasks.values(), return_exceptions=True
            )
