...
class AgentLoop:
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        async with lock, gate:
            try:
                on_stream_cb = cast(Callable[[str], Awaitable[None]] | None, None)
                on_stream_end_cb = cast(Callable[..., Awaitable[None]] | None, None)
        ...
        *,
        ...
        for block in content:
            ...
            if block.get("type") == "text" and isinstance(block.get("text"), str):
                text = block["text"]
                if truncate_content and len(text) > self.max_tool_result_chars:
