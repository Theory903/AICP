...
def test_cmd(url: str) -> None:
    ...
    # Test 2: Health
    results.append(
        _test_endpoint("App Health", "GET", f"{url}/health", expect_status=200)
    )

    # Test 3: V1 capabilities rank
    results.append(
        _test_endpoint("Rank", "GET", f"{url}/v1/capabilities/rank?query=grades")
    )
    ...
    # Test 9: Console
    results.append(
        _test_endpoint("Console", "GET", f"{url}/console", expect_status=200)
    )
    ...
    results.append(
        _test_endpoint(
            ...
            "POST",
            f"{url}/v1/execute",
            data={"capability_name": "grades.list", "arguments": {}},
        )
    )
...
def _test_endpoint(
    ...
    try:
        ...
        if r.status_code == expect_status or (
            expect_status == 200 and r.status_code < 400
        ):
...
@click.option("--url", "-u", default="http://127.0.0.1:8000", help="Runtime URL")
@click.option(
    "--decision", "-d", default="approved", help="Decision (approved/rejected)"
)
