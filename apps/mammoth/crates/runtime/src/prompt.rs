...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("MAMMOTH.md"),
            dir.join("MAMMOTH.local.md"),
            dir.join(".mammoth").join("MAMMOTH.md"),
            dir.join(".mammoth").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Mammoth instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Mammoth instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Mammoth Code settings files loaded.".to_string(),
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".mammoth")).expect("nested mammoth dir");
        fs::write(root.join("MAMMOTH.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("MAMMOTH.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".mammoth")).expect("apps mammoth dir");
        fs::write(root.join("apps").join("MAMMOTH.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".mammoth").join("instructions.md"),
            "apps dot mammoth instructions",
        )
        .expect("write apps dot mammoth instructions");
        fs::write(nested.join(".mammoth").join("MAMMOTH.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".mammoth").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("MAMMOTH.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("MAMMOTH.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.mammoth/MAMMOTH.md")),
            "MAMMOTH.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("MAMMOTH.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? MAMMOTH.md"));
    ...
    #[test]
    fn load_system_prompt_reads_mammoth_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".mammoth")).expect("mammoth dir");
        fs::write(root.join("MAMMOTH.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".mammoth").join("settings.json"),
        ...
        let original_mammoth_home = std::env::var("MAMMOTH_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("MAMMOTH_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_mammoth_home {
            std::env::set_var("MAMMOTH_CONFIG_HOME", value);
        } else {
            std::env::remove_var("MAMMOTH_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_mammoth_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".mammoth")).expect("mammoth dir");
        fs::write(root.join("MAMMOTH.md"), "Project rules").expect("write MAMMOTH.md");
        fs::write(
            root.join(".mammoth").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Mammoth instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_mammoth_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".mammoth")).expect("nested mammoth dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".mammoth/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/MAMMOTH.md"),
