...
impl Default for AicpConfig {
    fn default() -> Self {
        Self {
            url: "http://localhost:10003".to_string(),
            enabled: false,
            trust_tier: "2".to_string(),
            session_id: None,
        }
    }
}
...
impl AicpConfig {
    ...
    ///
    /// Checks `AICP_URL` environment variable first; falls back to the
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".mammoth.json"),
            |parent| parent.join(".mammoth.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".mammoth.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".mammoth").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".mammoth").join("settings.local.json"),
            },
        ]
    }
...
impl RuntimeConfig {
    ...
    #[must_use]
    pub fn aicp(&self) -> Option<&AicpConfig> {
        self.feature_config.aicp.as_ref()
    }
}
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("MAMMOTH_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".mammoth")))
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".mammoth.json");
...
fn parse_optional_aicp_config(root: &JsonValue) -> Result<Option<AicpConfig>, ConfigError> {
    let Some(object) = root.as_object() else {
        return Ok(None);
    };
    let Some(aicp_value) = object.get("aicp") else {
        return Ok(None);
    };
    let aicp = expect_object(aicp_value, "merged settings.aicp")?;
    let url = optional_string(aicp, "url", "merged settings.aicp")?
        .unwrap_or("http://localhost:10003")
        .to_string();
    let enabled = optional_bool(aicp, "enabled", "merged settings.aicp")?.unwrap_or(false);
    let trust_tier = optional_string(aicp, "trustTier", "merged settings.aicp")?
        .unwrap_or("2")
        .to_string();
    let session_id =
        optional_string(aicp, "sessionId", "merged settings.aicp")?.map(ToString::to_string);
    Ok(Some(AicpConfig {
        url,
        enabled,
        trust_tier,
...
mod tests {
    ...
    #[test]
    fn loads_and_merges_mammoth_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".mammoth");
        ...
        fs::write(
            home.parent().expect("home parent").join(".mammoth.json"),
        ...
        fs::write(
            cwd.join(".mammoth.json"),
            r#"{"model":"project-compat","env":{"B":"2"}}"#,
        )
        .expect("write project compat config");
        fs::write(
            cwd.join(".mammoth").join("settings.json"),
        ...
        assert_eq!(MAMMOTH_SETTINGS_SCHEMA_NAME, "SettingsSchema");
