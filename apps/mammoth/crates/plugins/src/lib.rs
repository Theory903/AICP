...
const MANIFEST_RELATIVE_PATH: &str = ".mammoth-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("MAMMOTH_PLUGIN_ID", &self.plugin_id)
            .env("MAMMOTH_PLUGIN_NAME", &self.plugin_name)
            .env("MAMMOTH_TOOL_NAME", &self.definition.name)
            .env("MAMMOTH_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("MAMMOTH_PLUGIN_ROOT", root.display().to_string());
        }
...
fn plugin_manifest_path(root: &Path) -> Result<PathBuf, PluginError> {
    ...
    for candidate in [&direct_path, &packaged_path, &legacy_packaged_path] {
        if candidate.exists() {
...
mod tests {
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(
            &script_path,
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$MAMMOTH_PLUGIN_ID\" \"$MAMMOTH_TOOL_NAME\" \"$INPUT\"\n",
        );
        #[cfg(unix)]
        {
    ...
    #[test]
    fn load_plugin_from_directory_supports_legacy_packaged_manifest_path() {
        let root = temp_dir("manifest-legacy-packaged");
        write_file(
            root.join("hooks").join("pre.sh").as_path(),
            "#!/bin/sh\nprintf 'pre'\n",
        );
        write_file(
            root.join(LEGACY_MANIFEST_RELATIVE_PATH).as_path(),
            r#"{
  "name": "legacy-packaged-demo",
  "version": "1.0.0",
  "description": "legacy packaged plugin",
  "hooks": {
    "PreToolUse": ["./hooks/pre.sh"]
  }
}"#,
        ...
        let manifest =
            load_plugin_from_directory(&root).expect("legacy packaged manifest should load");
        assert_eq!(manifest.name, "legacy-packaged-demo");
