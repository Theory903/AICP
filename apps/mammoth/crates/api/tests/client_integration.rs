...
async fn send_message_posts_json_and_parses_response() {
    ...
    let body = concat!(
        ...
        "\"type\":\"message\",",
        "\"role\":\"assistant\",",
        "\"content\":[{\"type\":\"text\",\"text\":\"Hello from Mammoth\"}],",
    ...
    assert_eq!(
        ...
        vec![OutputContentBlock::Text {
            text: "Hello from Mammoth".to_string(),
        }]
    );
...
async fn live_stream_smoke_test() {
    ...
    let mut stream = client
        .stream_message(&MessageRequest {
            model: std::env::var("MAMMOTH_MODEL").unwrap_or_else(|_| "claude-sonnet-4-6".to_string()),
            max_tokens: 32,
