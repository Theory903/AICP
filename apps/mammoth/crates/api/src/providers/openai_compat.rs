...
impl StreamState {
    fn new(model: String) -> Self {
        Self {
            ...
            text_state: TextStreamState::NotStarted,
            finished: false,
            stop_reason: None,
            usage: None,
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Vec<StreamEvent> {
        ...
        for choice in chunk.choices {
            if let Some(content) = choice.delta.content.filter(|value| !value.is_empty()) {
                if self.text_state == TextStreamState::NotStarted {
                    self.text_state = TextStreamState::Started;
    ...
    fn finish(&mut self) -> Vec<StreamEvent> {
        ...
        let mut events = Vec::new();
        if self.text_state == TextStreamState::Started {
            self.text_state = TextStreamState::Finished;
            events.push(StreamEvent::ContentBlockStop(ContentBlockStopEvent {
                index: 0,
            }));
        }
