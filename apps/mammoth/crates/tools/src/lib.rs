...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set Mammoth Code settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("mammoth-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("MAMMOTH_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("MAMMOTH_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".mammoth-todos.json"))
}
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("mammoth-agent-{}", job.manifest.agent_id);
...
impl ProviderRuntimeClient {
    ...
    async fn stream_message_events(
        &self,
...
impl ApiClient for ProviderRuntimeClient {
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        self.runtime.block_on(async {
            let events = self.stream_message_events(&message_request).await?;
            if events
                .iter()
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("MAMMOTH_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".mammoth-agents"));
    }
    Ok(cwd.join(".mammoth-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "mammothguide" | "mammothguideagent" | "guide" => String::from("mammoth-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".mammoth").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("MAMMOTH_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".mammoth"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("mammoth-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::remove_var("MAMMOTH_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("MAMMOTH_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("MAMMOTH_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("MAMMOTH_TODO_STORE");
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("MAMMOTH_AGENT_STORE", &dir);
        ...
        std::env::remove_var("MAMMOTH_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".mammoth")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".mammoth")).expect("cwd dir");
        std::fs::write(
            home.join(".mammoth").join("settings.json"),
            r#"{"verbose":false}"#,
        )
        .expect("write global settings");

        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("MAMMOTH_CONFIG_HOME").ok();
        let original_dir = std::env::current_dir().expect("cwd");
        ...
        match original_config_home {
            Some(value) => std::env::set_var("MAMMOTH_CONFIG_HOME", value),
            None => std::env::remove_var("MAMMOTH_CONFIG_HOME"),
        }
