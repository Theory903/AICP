...
@click.option(
    ...
    default=50,
    show_default=True,
    help="Maximum approvals to display.",
)
@click.option(
    "--server",
    "-u",
...
@click.argument("approval_id")
@click.option(
    "--approver", "-a", default="cli-user", show_default=True, help="Approver identity."
)
@click.option("--reason", "-r", help="Approval reason.")
@click.option("--server", "-u", "server_url", help="AICP server URL")
def appr_ok(
    approval_id: str, approver: str, reason: str | None, server_url: str | None
) -> None:
...
@click.argument("approval_id")
@click.option(
    "--approver", "-a", default="cli-user", show_default=True, help="Rejector identity."
)
...
async def _run_appr_list(status: str, limit: int, server_url: str | None) -> None:
    ...
    if status != "all":
        normalized = [
            item for item in normalized if item.get("status", "pending") == status
        ]
...
async def _run_appr_list_server(client: Any, status: str, limit: int) -> None:
    ...
    try:
        ...
        raise click.ClickException(
            f"Failed to load approvals from server: {exc}"
        ) from exc
...
async def _run_appr_decide(
    ...
    try:
        ...
        raise click.ClickException(
            f"Failed to record {decision} decision: {exc}"
        ) from exc

    status_word = "Approved" if decision == "approved" else "Rejected"
    click.secho(
        f"{status_word} [{approval_id}]",
        fg="green" if decision == "approved" else "red",
    )
