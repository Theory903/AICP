...
class PlaywrightPerceptionProvider(PerceptionProvider):
    ...
    async def get_accessibility_tree(self, url: str) -> AccessibilityTree:
        ...
        try:
            ...
            try:
                ...
                root_id = next((node_id for node_id, node in nodes.items() if node.get("role", {}).get("value") == "RootWebArea"), None)
            ...
            def has_role(node: Dict[str, Any], target: str) -> bool:
    ...
    def _map_a11y_node(self, node: Dict[str, Any], parent_id: Optional[str] = None) -> A11yElement:
        children = []
        if "children" in node:
    ...
    async def get_dom_snapshot(self, url: str) -> DOMSnapshot:
        ...
        try:
            ...
            forms = await page.evaluate("() => Array.from(document.forms).map(f => ({ id: f.id, action: f.action }))")
            inputs = await page.evaluate("() => Array.from(document.querySelectorAll('input, select, textarea')).map(i => ({ id: i.id, type: i.type, name: i.name }))")
    ...
    async def execute_action(self, url: str, action_type: InteractionType, target_id: str, value: Optional[str] = None) -> bool:
        await self._ensure_browser()
        if not self._browser:
            raise RuntimeError("Browser not initialized")
        context = await self._browser.new_context()
