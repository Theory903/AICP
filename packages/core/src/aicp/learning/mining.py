...
from .learning import LearningService, Skill, MinedSkill, DriftDetector, DriftType, DriftDetection
...
class TfIdfSkillMiner:
    ...
    def mine_skills(self, min_support: int = 2) -> List[MinedSkill]:
        ...
        for session in self.sessions:
            unique_caps = sorted(list(set(session)))
...
class BayesianDriftDetector(DriftDetector):
    ...
    def detect_drift(self, metric_name: str, current_value: float, threshold_sigma: float = 2.0) -> Optional[DriftDetection]:
        """
        ...
        if z_score > threshold_sigma:
            severity = "high" if z_score > threshold_sigma * 2 else "medium"
