...
class VoiceService:
    ...
    async def _transcribe_openai(
        ...
        try:
            ...
            client = OpenAI(api_key=api_key, base_url=self._get_base_url(VoiceProvider.OPENAI))
    ...
    async def _synthesize_openai(self, text: str, voice_id: str) -> VoiceSynthesisResult:
        api_key = self._get_api_key(VoiceProvider.OPENAI)
        if not api_key:
            raise VoiceProviderError("OpenAI API key not configured")
        try:
    ...
    async def _synthesize_elevenlabs(self, text: str, voice_id: str) -> VoiceSynthesisResult:
