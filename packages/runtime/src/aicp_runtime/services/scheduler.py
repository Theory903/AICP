    ...
    def _parse_field(cls, value: str, field: str) -> list[int]:
        ...
        for part in value.split(","):
            if "/" in part:
                ...
                step = int(step)
                rng = cls._get_range(field) if base == "*" else cls._parse_range(base, field)
...
class SchedulerService:
    ...
    def _persist_schedule(self, schedule: ScheduleConfig) -> None:
        ...
        conn.execute(
            ...
            (schedule.schedule_id, schedule.model_dump_json(), datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()
        conn.close()
    ...
    def create_schedule(self, config: ScheduleConfig) -> ScheduleConfig:
        ...
        if config.schedule_type == ScheduleType.ONE_TIME and not config.one_time_timestamp:
        ...
        if config.schedule_type == ScheduleType.CRON and config.cron_expression:
            ...
            config.next_run = datetime.now(timezone.utc) + timedelta(seconds=float(ival))
        else:
    ...
    def trigger_now(self, schedule_id: str) -> dict[str, Any]:
        ...
        if self._executor:
            ...
            schedule.last_result = {"status": "success", "execution_id": result.get("execution_id")}
        else:
    ...
    def _update_next_run(self, schedule: ScheduleConfig) -> None:
        if schedule.schedule_type == ScheduleType.CRON and schedule.cron_expression:
            try:
                schedule.next_run = CronParser.get_next_run(schedule.cron_expression, schedule.last_run)
            ...
            if schedule.interval_seconds is None:
                ...
                schedule.next_run = datetime.now(timezone.utc) + timedelta(seconds=float(ival))
        else:
            schedule.next_run = None
            schedule.state = ScheduleState.COMPLETED
        schedule.updated_at = datetime.now(timezone.utc)
