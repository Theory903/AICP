...
class BrowserService:
    ...
    async def create_context(self, session_id: str, headless: bool = True, viewport: Optional[Viewport] = None) -> str:
        ...
        self._contexts[session_id] = {"context": context, "page": page, "headless": headless}
        return session_id
    ...
    async def type_text(self, session_id: str, selector: str, text: str) -> BrowserResult:
        if session_id not in self._contexts:
            raise BrowserError(f"No context for session: {session_id}")
        page = self._contexts[session_id]["page"]
        try:
    ...
    async def wait_for(self, session_id: str, selector: str, timeout_ms: int = 10000) -> BrowserResult:
        if session_id not in self._contexts:
            raise BrowserError(f"No context for session: {session_id}")
        page = self._contexts[session_id]["page"]
        try:
