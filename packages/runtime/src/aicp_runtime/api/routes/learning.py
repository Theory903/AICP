...
def build_learning_router(learning_service: LearningService) -> APIRouter:
    ...
    @router.post("/benchmarks/results", response_model=BenchmarkResult, status_code=status.HTTP_201_CREATED)
    async def record_benchmark_result(result: BenchmarkResult) -> BenchmarkResult:
        learning_service.record_benchmark_result(result)
        return result

    @router.get("/benchmarks/{benchmark_id}/results", response_model=List[BenchmarkResult])
